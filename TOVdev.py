...
#Speed of sound
def speedOFsound(P):
    return np.sqrt(5/3 * k * RhoEQS(P)**(2/3))
...
# Initialisation ===================================================================================
    ...
    def PlotGttGrr(self):
        ...
        if self.dilaton == True:
            label_plus = 'ER'
        else:
            label_plus = 'GR'
        plt.plot([x/10**3 for x in self.radius], [x * y for x,y in zip(self.metric00,self.metric11)], label = f'a * b ({label_plus})')
        #plt.plot([x/10**3 for x in self.radius[self.stepStar:]], [x * y for x,y in zip(self.metric00[self.stepStar:],self.metric11[self.stepStar:])], label = '$g_{00}$ x $g_{11}$')
        value_radius = self.radius[self.stepStar]
        plt.axvline(x = value_radius /10**3, color = 'red', label = f'radius ({label_plus})')
        plt.legend(loc = 'lower right')
        plt.savefig(f'figures_GttGrr_{label_plus}/Fig_GttGrr_{label_plus}_{int(self.initDensity/(cst.eV*10**6/(cst.c**2*cst.fermi**3)))}.png', dpi = 200)
        plt.show()

    def PlotGrroverGtt(self):
        ...
        if self.dilaton == True:
            ...
            label_plus = 'GR'
        plt.plot([x/10**3 for x in self.radius], [y / x for x,y in zip(self.metric00,self.metric11)], label = f' b / a ({label_plus})')
        #plt.plot([x/10**3 for x in self.radius[self.stepStar:]], [x * y for x,y in zip(self.metric00[self.stepStar:],self.metric11[self.stepStar:])], label = '$g_{00}$ x $g_{11}$')
        value_radius = self.radius[self.stepStar]
        plt.axvline(x = value_radius /10**3, color = 'red', label = f'radius ({label_plus})')
        plt.legend(loc = 'lower right')
        plt.savefig(f'figures_GrroverGtt_{label_plus}/Fig_GrroverGtt_{label_plus}_{int(self.initDensity/(cst.eV*10**6/(cst.c**2*cst.fermi**3)))}.png', dpi = 200)
        plt.show()

    def plot_speedOFsound(self):
        plt.figure()
        plt.xlabel('Radius r (km)')
        plt.ylabel('c_s/c')
        if self.dilaton == True:
            ...
            label_plus = 'GR'
        plt.plot([x/10**3 for x in self.radius], [speedOFsound(x) / cst.c for x in self.pressure], label = f' c_s/c ({label_plus})')
        plt.axhline(y = 1, color = 'red', label = f'Causal limit')

        plt.legend()
        plt.savefig(f'figures_csoverc_{label_plus}/Fig_csGrroverc_{label_plus}_{int(self.initDensity/(cst.eV*10**6/(cst.c**2*cst.fermi**3)))}.png', dpi = 200)
        plt.show()
